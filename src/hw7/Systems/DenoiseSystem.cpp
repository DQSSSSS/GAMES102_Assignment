...
void DenoiseSystem::OnUpdate(Ubpa::UECS::Schedule& schedule) {
	schedule.RegisterCommand([](Ubpa::UECS::World* w) {
		...
		if (ImGui::Begin("Denoise")) {
			...
			ImGui::Text("Param Shape:");
			ImGui::SameLine();
			ImGui::RadioButton("Non", &data->param_shape, 0); // 
			ImGui::SameLine();
			ImGui::RadioButton("Circle", &data->param_shape, 1); //
			ImGui::SameLine();
			ImGui::RadioButton("Square", &data->param_shape, 2); //

			ImGui::Text("Param Method:");
			ImGui::SameLine();
			ImGui::RadioButton("Uniform", &data->param_method, 0); // 
			ImGui::SameLine();
			ImGui::RadioButton("Distance", &data->param_method, 1); //

			ImGui::Text("Global Method:");
			ImGui::SameLine();
			ImGui::RadioButton("Constant", &data->global_type, 0); // 
			ImGui::SameLine();
			ImGui::RadioButton("Cotan", &data->global_type, 1); //
			...
			if (ImGui::Button("Global")) {
				...
				ParamShape shape;
				if (data->param_shape == 0) shape = ParamShape::NON;
				else if(data->param_shape == 1) shape = ParamShape::CIRCLE;
				else if(data->param_shape == 2) shape = ParamShape::SQUARE;

				ParamMethod method;
				if (data->param_method == 0) method = ParamMethod::UNIFORM;
				else if (data->param_method == 1) method = ParamMethod::DISTANCE;

				GlobalType gt;
				if (data->global_type == 0) gt = GlobalType::CONSTANT;
				else if (data->global_type == 1) gt = GlobalType::COT;

				auto boundarys = parametrization(data->heMesh, shape, method, data->param_radius);
				globalMinizing(data->heMesh, boundarys, gt);
			...
			ImGui::Text("Color:");
			ImGui::SameLine();
			ImGui::RadioButton("White", &data->color_type, 0); // 
			ImGui::SameLine();
			ImGui::RadioButton("Normal", &data->color_type, 1); //
			ImGui::SameLine();
			ImGui::RadioButton("Laplace", &data->color_type, 2); //
			ImGui::SameLine();
			ImGui::RadioButton("Gauss", &data->color_type, 3); // 
